...
TEST_PROGRAM="test_posix" # test_falcon / test_posix
MOUNT_DIR="/test/" # meta directory / falconfs mount path, end with /
FILE_PER_THREAD=200
PORT=1111
FILE_SIZE=1572864
CLIENT_NUM=1
THREAD_NUM_PER_CLIENT=4000
ROUND_INDEX=(0 1 2 3)
...
do
    SERVER_IP=$META_SERVER_IP SERVER_PORT=$META_SERVER_PORT READ_STANDBY=$READ_META_STANDBY LD_LIBRARY_PATH=/usr/local/lib64/:$LD_LIBRARY_PATH $BIN_DIR/$TEST_PROGRAM $MOUNT_DIR $FILE_PER_THREAD $THREAD_NUM_PER_CLIENT $round_idx $CLIENT_ID $MOUNT_PER_CLIENT $CLIENT_CACHE_SIZE $PORT $FILE_SIZE $CLIENT_NUM > $BIN_DIR/result_1111 2>&1 &
    sleep 1
